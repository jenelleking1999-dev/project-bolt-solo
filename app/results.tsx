...
async function fetchLatestSession(userId: string | undefined) {
  let query = supabase
    .from('sessions')
...
export default function ResultsScreen() {
  ...
  useEffect(() => {
    if (authLoading && !params.sessionId && !params.workoutId) return;
    ...
    (async () => {
      ...
      // PRIMARY: workoutId from navigation params (per strict requirement #6)
      if (params.workoutId) {
        console.log('[Results] Fetching by workoutId:', params.workoutId);
        let query = supabase
          .from('sessions')
          .select('id')
        ...
        // If a sessionId is also provided, use it to pick the exact session
        if (params.sessionId) {
          ...
          if (exact) resolvedSessionId = exact.id;
        }
        ...
        // SECONDARY: direct sessionId
        resolvedSessionId = params.sessionId;
      }

      // FALLBACK (only if workoutId is missing): most recent completed workout
      if (!resolvedSessionId) {
        console.log('[Results] Fallback: fetching most recent completed workout');
        const { data: latestSession } = await fetchLatestSession(userId);

        if (id !== fetchId.current) return;

        if (!latestSession) {
          console.log('[Results] No completed workout found');
      ...
      console.log('[Results] Loaded workout:', details.workout?.name, '— splits:', details.splits.length);
  ...
  }, [params.sessionId, params.workoutId, params.refresh, params.refreshKey, user?.id, authLoading]);
  ...
  const handleExportToExcel = () => {
    ...
    const excelData = exportSingleWorkoutToExcel({
      sessionId: activeSessionId,
      workoutName: workout.name,
      date: new Date().toLocaleDateString(),
      splits,
      targetTimeSeconds: workout.target_time,
    });
    const filename = `${workout.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.emptyState}>
          <Text style={styles.emptyStateTitle}>No workouts completed yet</Text>
      ...
      <View style={styles.header}>
        ...
        {ENABLE_DOWNLOAD ? (
          <TouchableOpacity onPress={handleExportToExcel} style={styles.headerBtn}>
            <Download size={22} color={colors.dark.primary} />
          </TouchableOpacity>
        ) : (
          <View style={styles.headerBtn} />
        )}
